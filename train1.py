...
import pandas as pd
import torch
from ckpt import modelM as model
import os
import datetime
...
from loss import loss_contrast
...
def main():
    ...
    # ************************************************ 2 **********************************************
    # ================================= load train data =============================
    train_set_label = datasetList.data_set(csv_path='./dataset/dataset_csv/data_all_v1.csv',
    ...
    print(device)
    model_net = model.Network(opt.dtmConvChannel_in, opt.dtmConvChannel_out, device)
    print("1: ", model_net)
    ...
    # loss_function = focalLoss.FocalLoss(logits=True)
...
def train_label(model_net, EPOCH, initial_epoch, train_loader_label, loss_function, optimizer, scheduler, device):
    for epoch in range(initial_epoch, EPOCH):
        ...
        for i, data1 in enumerate(train_loader_label):
            ...
            contrast_focus, outputs_focus = model_net(data1_img)
...
def train_fusion(train_model, image1_path, image2_path, decisionMap_savePath, fusionMap_savePath, fusionMap_re_savePath, device):
    ...
    contrast_focus, output_focus = train_model(image_device)
